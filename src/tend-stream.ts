...
export class TendStream extends EventEmitter {
  ...
  private jwt: string;
  private cam: TendCameraInfo;
  private userAlias = "";
  private cxs: CxClient | null = null;
  private udp: dgram.Socket | null = null;
  private udpAudio: dgram.Socket | null = null;
  private relayHost = "";
  private relayPort = 0;
  private cameraPubIp = "";
  private cameraLanIp = "";
  private cameraPort = 0;
  private cameraAudioPort = 0;
  private cameraAudioLanIp = "";
  private cameraAudioPubIp = "";
  private localIp = "";
  private localPort = 0;
  private localAudioPort = 0;
  private block1: Buffer = Buffer.alloc(32);
  private block2: Buffer = Buffer.alloc(32);
  private aesKey: Buffer;
  private resCounter = 0;
  private incomingReqPunchCount = 0;
  private preferLan = false;
  private sentSeq2 = false;
  private sentSeq2Audio = false;
  private punchInterval: NodeJS.Timeout | null = null;

  // Per-frame_id reassembly buffers
  private assembly = new Map<number, FrameAssembly>();
  private maxAssemblyAge = 2000; // ms

  // H.264 emit
  private h264Buffer = Buffer.alloc(0);

  // Stats
  public packetsRx = 0;
  public framesDecoded = 0;
  public nalsEmitted = 0;
  public decryptFailures = 0;
  ...
  public async start(): Promise<void> {
    ...
    const videoTargets: Array<[string, number]> = [[this.cameraPubIp, this.cameraPort]];
    const audioTargets: Array<[string, number]> = [[this.cameraAudioPubIp, this.cameraAudioPort]];
  ...
  private buildReqConnInfo(channelId: number, localPort: number): Buffer {
  ...
  private buildAckConnInfo(channelId: number): Buffer {
  ...
  private buildReqPunch(seq: number): Buffer {
  ...
  private buildResPunch(counter: number): Buffer {
  ...
  private buildAckPunch(echo: number): Buffer {
  ...
  private maybeSendPunchResponse(): void {
  ...
  private onUdp(msg: Buffer, rinfo: dgram.RemoteInfo, isAudioChannel: boolean): void {
  ...
  private onMediaChunk(raw: Buffer): void {
  ...
  private processAssembled(_frameId: number, a: FrameAssembly): void {
  ...
  private aesDecrypt(iv: Buffer, ciphertext: Buffer): Buffer | null {
  ...
  private emitNals(plaintext: Buffer): void {
  ...
  private static buildBlock(prefix: string, alias: string): Buffer {
