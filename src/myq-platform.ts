...
import { MYQ_ACTIVE_DEVICE_REFRESH_DURATION, MYQ_ACTIVE_DEVICE_REFRESH_INTERVAL, MYQ_API_BACKOFF_LADDER, MYQ_DEVICE_REFRESH_INTERVAL, MYQ_MQTT_TOPIC,
    ...
    for(const device of this.myQApi.devices) {
      ...
      if(!accessory) {
        ...
        const accessoryCategory = device.device_family === "lamp"
          ...
          ? this.hap.Categories.LIGHTBULB
          : this.hap.Categories.GARAGE_DOOR_OPENER;
        ...
        this.api.registerPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, [accessory]);
        this.accessories.push(accessory);
      } else if(device.device_family.indexOf("garagedoor") !== -1 && accessory.category !== this.hap.Categories.GARAGE_DOOR_OPENER) {
      ...
      void (async (): Promise<void> => {
        ...
        if(!(await this.updateAccessories())) {
          ...
          this.log.warn("myQ API unreachable (%s consecutive failure%s). Backing off — next attempt in %s seconds — to avoid hammering the myQ service.",
            this.pollFailures, this.pollFailures === 1 ? "" : "s", backoff);
