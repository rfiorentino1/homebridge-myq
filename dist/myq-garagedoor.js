...
export class myQGarageDoor extends myQAccessory {
    ...
    configureDevice() {
        ...
        const doorInitialState = this.accessory.context.doorState;
    ...
    configureSwitch() {
        ...
        switchService.getCharacteristic(this.hap.Characteristic.On)?.onSet((isOn) => {
            // Inform the user.
    ...
    configureOccupancySensor() {
        ...
        this.log.info("Enabling the open indicator occupancy sensor. Occupancy will be triggered when the opener has been continuously open for more than %s seconds.", this.hints.occupancyDuration);
        return true;
    }
    // Configure MQTT.
    configureMqtt() {
        // Return the current status of the garage door.
        this.platform.mqtt?.subscribe(this.accessory, this.myQ, "garagedoor/get", (message) => {
        ...
        // Return the current status of the garage door.
        this.platform.mqtt?.subscribe(this.accessory, this.myQ, "garagedoor/set", (message) => {
            const value = message?.toString()?.toLowerCase();
            let targetName;
            let targetState;
    ...
    setDoorState(value) {
        ...
        if ((this.status === this.hap.Characteristic.CurrentDoorState.OPENING) || (this.status === this.hap.Characteristic.CurrentDoorState.CLOSING)) {
            this.log.error("Unable to %s door while currently attempting to complete %s. myQ must complete it's existing action before attempting a new one.", actionAttempt, actionExisting);
            return false;
        }
    ...
    updateState() {
        ...
        const oldState = this.accessory.context.doorState;
    ...
    async doorCommand(command) {
        let myQCommand;
        let myQRevertCurrentState;
        let myQRevertTargetState;
...
//# sourceMappingURL=myq-garagedoor.js.map